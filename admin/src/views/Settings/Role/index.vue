<template>
  <x-panel :items="panel_1">
    <x-toolbar slot="toolbar" ref="toolbar"
               ...
               :url="toolbar_url1"
               :showChildrenNum="5"
               @submit-success#update="toolbar1" @confirm-success#delete="toolbar2">
    </x-toolbar>
    <x-table slot="grid" ref="grid"
...
export default {
  ...
  data () {
    return {
      ...
      toolbar_url1: '/ToolbarMetadata/Settings/Role',
      toolbar_children1: [],
      grid_loadColumns1: true,
      grid_columnsUrl1: '/GridMetadata/Settings/Role',
  ...
  methods: {
    toolbar1 () {
      this.$refs['grid'].loadDataHandler()
    },
    toolbar2 () {
      this.$refs['grid'].loadDataHandler()
    },
    grid_rowclick1 (row, event, column) {
      ...
      this.$refs['toolbar'].trigger('privilege', 'setLoadModelParams', {roleId: row.id})
    }
  },
  timers: []
}
